...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rate_resto'])) {
    if (!$user_logged_in) {
    ...
    $check_query = "SELECT * FROM resto_ratings WHERE resto_id = '$resto_id' AND uname = '$username'";
    $check_result = $conn->query($check_query);

    if ($check_result->num_rows > 0) {
        // If rating exists, update it
        $update_query = "UPDATE resto_ratings SET rating = '$rating' WHERE resto_id = '$resto_id' AND uname = '$username'";
        $conn->query($update_query);
        echo json_encode(["status" => "success", "message" => "Your rating has been updated!"]);
    } else {
        // If no rating exists, insert new
        $insert_query = "INSERT INTO resto_ratings (uname, resto_id, rating) VALUES ('$username', '$resto_id', '$rating')";
...
<div class="container mt-5">
    ...
    <div class="row">
        <?php
        $sql = "SELECT r.*, 
                (SELECT AVG(rating) FROM resto_ratings WHERE resto_id = r.id) AS avg_rating,
                (SELECT rating FROM resto_ratings WHERE resto_id = r.id AND uname = '$username' LIMIT 1) AS user_rating 
                FROM resto r";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<a href="resto-details.php?id=' . $row['id'] . '" class="btn btn-secondary me-2">View More</a>';
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Rate This Restaurant</h5>
            ...
            <div class="modal-body">
                <?php if ($user_logged_in): ?>
                    <form id="ratingForm">
                        <input type="hidden" id="resto_id" name="resto_id">
                ...
                <?php else: ?>
                    <p>You must <a href="register.php">create an account</a> or <a href="login.php">log in</a> to rate this restaurant.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<script>
function openRatingModal(restoId, userRating) {
    document.getElementById("resto_id").value = restoId;
    document.getElementById("rating").value = userRating || 1;
